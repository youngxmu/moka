...
exports.insertUserExam = function (uid, exam_id, callback) {
    var sql = 'insert into user_exam (uid, exam_id, status, create_time) values(?,?,?,?);';
    var params = [];
    params.push(uid);
    params.push(exam_id);
    params.push(1);
...
exports.queryUserExam = function (uid, exam_id, callback) {
    var sql = 'select * from user_exam where uid=? end exam_id = ?;';
    var params = [];
    params.push(uid);
    params.push(exam_id);
    db.query(sql, params, callback);
...
/** user_exam **/
exports.updateUserExamStatus = function (uid, exam_id, status, callback) {
    var sql = 'update user_exam set status = 0 where uid = ? end exam_id = ?';
    var params = [];
    params.push(uid);
    params.push(exam_id);
    db.query(sql, params, callback);
...
/** history */
exports.insertExamHistory = function (uid, exam_id, answer, callback) {
    var sql = 'insert into exam_history (uid, exam_id, answer, create_time) values(?,?,?,?);';
    db.query(sql,[uid, exam_id, answer, new Date()], callback);
};

exports.queryExamHistorys = function (uid, start, pageSize, callback) {
    var sql = 'select h.*,p.name,p.description from exam_history h left join exam p on h.pid = p.id where h.uid = ? order by create_time desc ';
    var params = [];
    sql += ' limit ?,?;';
    params.push(uid);
    params.push(start);
...
exports.getExamHistoryById = function (id, callback) {
    db.query("select h.*,p.name,p.description from exam_history h left join exam p on h.pid = p.id  where h.id = ?;", [id], function (err, result) {
        if(!err && result && result[0]){
            callback(err, result[0]);
        }else{
            callback(err);    
